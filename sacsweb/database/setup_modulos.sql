...
INSERT INTO `modulos` 
(`titulo`, `descricao`, `nivel`, `tipo_ataque`, `conteudo_teorico`, `exemplo_pratico`, `demonstracao_codigo`, `linguagem_codigo`, `tempo_estimado`, `pontos_maximos`, `ordem`, `ativo`) VALUES
('Introdução à Segurança da Web',
'Fundamentos básicos sobre vulnerabilidades web e como elas funcionam',
'iniciante',
'Geral',
'O que é uma vulnerabilidade\n\nEm segurança da informação, vulnerabilidade é uma fraqueza ou falha em um sistema — seja software, hardware, rede ou configuração — que pode permitir que alguém use isso de forma indevida, para causar dano, roubar dados ou assumir controle do sistema.\n\nVulnerabilidades podem existir em programas de computador, celulares, roteadores, servidores, equipamentos de rede e praticamente qualquer dispositivo conectado.\n\nElas são diferentes de ameaças: ameaça é o que pode atacar ou explorar; vulnerabilidade é o que está fraco ou acessível. Se não há vulnerabilidade, a ameaça não consegue agir.\n\n\nPor que vulnerabilidades acontecem\n\n1. **Erros de programação (bugs)**\nProgramadores podem escrever código com falhas, sem prever todas as formas que um usuário ou atacante pode explorar. Por exemplo, não validar corretamente os dados que o usuário envia.\n\n2. **Design ou planejamento ruim**\nMesmo antes de escrever o código, se o sistema não for pensado com segurança — como definir permissões e formas de armazenar senhas —, já podem surgir brechas.\n\n3. **Configurações incorretas ou padrões inseguros**\nMuitos softwares vêm configurados de forma genérica ou com privilégios excessivos. Se essas configurações não forem ajustadas, criam aberturas para ataques.\n\n4. **Software desatualizado**\nQuando não aplicamos atualizações (patches) que corrigem falhas já conhecidas, deixamos o sistema vulnerável a ataques simples.\n\n5. **Uso de componentes de terceiros vulneráveis**\nBibliotecas, plugins e frameworks externos podem conter falhas que afetam todo o sistema.\n\n6. **Fator humano**\nSenhas fracas, descuido, falta de conhecimento ou erros de configuração também causam vulnerabilidades.\n\n\nComo uma vulnerabilidade é explorada\n\n1. **Descoberta:** alguém identifica a falha (pesquisador, invasor ou fabricante).\n2. **Prova de conceito (PoC):** cria-se um teste controlado para ver se a falha pode ser explorada.\n3. **Exploração:** o invasor usa a vulnerabilidade para acessar dados, executar código ou controlar o sistema.\n4. **Propagação / Escalonamento:** o invasor tenta aumentar privilégios, acessar outras áreas ou sistemas conectados.\n5. **Consequências:** roubo de dados, indisponibilidade do serviço, perda de integridade ou instalação de malware.\n\n\nTipos básicos de vulnerabilidades comuns\n\n| Tipo | O que é (em termos simples) | Por que é perigoso |\n|------|-----------------------------|--------------------|\n| **Injeção de código (SQL Injection)** | Inserção de comandos maliciosos em campos de entrada de dados. | Pode permitir acesso, modificação ou exclusão de dados. |\n| **Cross-Site Scripting (XSS)** | Site que aceita entrada de usuário sem filtrar e exibe código malicioso (JavaScript, por exemplo). | Pode roubar cookies, sessões e redirecionar usuários. |\n| **Quebra de autenticação / sessão** | Falhas que permitem que invasores finjam ser usuários legítimos. | Permite roubo de identidade ou acesso indevido. |\n| **Controle de acesso quebrado** | O sistema não limita corretamente o que cada usuário pode fazer. | Usuários comuns acessam dados de administradores. |\n| **Configuração incorreta** | Padrões inseguros, portas abertas, permissões exageradas. | Facilita ataques e invasões. |\n| **Componentes vulneráveis** | Uso de bibliotecas ou softwares desatualizados com falhas conhecidas. | Permite ataques simples com exploits públicos. |\n| **Zero-day** | Falha desconhecida pelo fabricante e sem correção disponível. | Muito perigosa, pois não há defesa imediata. |\n\n\nConceitos auxiliares importantes\n\n- **Patch / atualização:** correção oficial para falhas de segurança.\n- **Exploit:** o método ou ferramenta usada para explorar uma vulnerabilidade.\n- **Superfície de ataque:** todas as partes acessíveis de um sistema — quanto maior, mais riscos.\n- **CIA (Confidencialidade, Integridade e Disponibilidade):** os três pilares da segurança.\n  - *Confidencialidade:* dados só visíveis por quem deve.\n  - *Integridade:* dados não alterados indevidamente.\n  - *Disponibilidade:* o sistema funciona quando necessário.\n- **CVE (Common Vulnerabilities and Exposures):** catálogo público de vulnerabilidades conhecidas.\n- **CVSS (Common Vulnerability Scoring System):** sistema que mede a gravidade de vulnerabilidades.\n\n\nExemplos simples para entender na prática\n\n- Um roteador com senha padrão “admin/admin” pode ser facilmente invadido.\n- Um site que exibe comentários sem filtrar pode permitir XSS, rodando código no navegador de outros.\n- Um sistema que não verifica permissões pode mostrar dados de outros usuários.\n- Um computador desatualizado continua com falhas já corrigidas por fabricantes.\n\n\nImpactos de uma vulnerabilidade não corrigida\n\n- Vazamento de dados pessoais e informações sensíveis.\n- Roubo de propriedade intelectual e prejuízo financeiro.\n- Queda na confiança de clientes e danos à reputação.\n- Processos legais por violar normas como LGPD e GDPR.\n- Paralisação de serviços e perda de produtividade.\n\n\nComo mitigar vulnerabilidades\n\n- **Atualize tudo regularmente:** sistemas, softwares e dispositivos.\n- **Configure bem desde o início:** evite padrões inseguros.\n- **Use senhas fortes e únicas.**\n- **Desative serviços não utilizados.**\n- **Restrinja acessos:** apenas o necessário.\n- **Valide entradas de dados:** nunca confie cegamente em entradas de usuário.\n- **Realize auditorias e testes de segurança** (como pentests e varreduras automáticas).\n- **Treine usuários e equipes** sobre segurança e boas práticas.\n- **Monitore sistemas continuamente** e reaja rápido a incidentes.\n- **Siga boas práticas reconhecidas**, como as da OWASP.\n\n\nConclusão\n\nUma vulnerabilidade é como uma porta aberta em um castelo: pode parecer pequena, mas basta para alguém entrar e causar estragos. Elas podem nascer de erros simples, más configurações ou descuido, mas quase sempre podem ser evitadas com atenção, atualização e boas práticas.',
'Vamos analisar um site real que foi hackeado para entender o impacto das vulnerabilidades. Sites como Yahoo, Equifax e outros grandes portais já foram comprometidos por falhas de segurança básicas.',
'<!DOCTYPE html> 
<html>
<head>
